# Storybook Migration — Specification
...
### TS-1: Setup verification
GIVEN Storybook initialized WHEN `npm run storybook` starts THEN it binds to port 6006. WHEN `npm run dev` starts separately THEN it binds to port 5173.

### TS-2: Story rendering
GIVEN Storybook is running WHEN navigating to each of the 4 component stories THEN each renders with default args without console errors.

### TS-3: Controls interaction
GIVEN a story with Controls panel open WHEN a user changes an arg value THEN the component re-renders with the new value.

### TS-4: Accessibility
GIVEN `@storybook/addon-a11y` installed WHEN running the A11y panel on each story THEN no "critical" severity violations are reported.

### TS-5: Build integrity
GIVEN the project AFTER Storybook installation WHEN `npm run build` executes THEN exit code is 0.

### TS-6: Test integrity
GIVEN the project AFTER Storybook installation WHEN `npm test` executes THEN all 42 existing tests pass AND no test files were modified.
