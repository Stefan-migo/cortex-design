...
/* ponytail: OGL WebGL flying posters — kept because the vertex shader
   distortion and 3D plane rotation are the core feature.
   Ceiling: OGL dependency, custom WebGL shaders, no SSR.
   Upgrade: use Three.js if more post-processing is needed. */
...
uniform float uPosition; uniform float uTime; uniform float uSpeed;
uniform vec3 distortionAxis; uniform vec3 rotationAxis; uniform float uDistortion;
varying vec2 vUv;
...
void main() {
  vUv = uv; float norm = 0.5; vec3 newpos = position;
...
precision highp float;
uniform vec2 uImageSize; uniform vec2 uPlaneSize; uniform sampler2D tMap;
varying vec2 vUv;
void main() {
  vec2 imageSize = uImageSize; vec2 planeSize = uPlaneSize;
  float imageAspect = imageSize.x/imageSize.y; float planeAspect = planeSize.x/planeSize.y;
  vec2 scale = vec2(1.0);
...
function map(num, min1, max1, min2, max2, round) {
  const n = ((num - min1) / (max1 - min1)) * (max2 - min2) + min2
...
function AutoBind(self) {
  ...
  Object.getOwnPropertyNames(proto).forEach(key => {
    if (key !== 'constructor' && typeof self[key] === 'function') self[key] = self[key].bind(self)
  })
}

class Media {
  constructor({ gl, geometry, scene, screen, viewport, image, length, index, planeWidth, planeHeight, distortion }) {
    this.gl = gl; this.geometry = geometry; this.scene = scene; this.screen = screen; this.viewport = viewport
    this.image = image; this.length = length; this.index = index
    this.planeWidth = planeWidth; this.planeHeight = planeHeight; this.distortion = distortion
    this.extra = 0; this.createShader(); this.createMesh(); this.onResize()
  }
  ...
  setScale() {
    ...
    this.plane.position.x = 0; this.program.uniforms.uPlaneSize.value = [this.plane.scale.x, this.plane.scale.y]
  }

  onResize({ screen, viewport } = {}) {
    if (screen) this.screen = screen
    if (viewport) { this.viewport = viewport; this.program.uniforms.uViewportSize.value = [viewport.width, viewport.height] }
    this.setScale(); this.padding = 5
  ...
  update(scroll) {
    ...
    this.program.uniforms.uPosition.value = position; this.program.uniforms.uTime.value += 0.04
...
class Canvas {
  constructor({ container, canvas, items, planeWidth, planeHeight, distortion, scrollEase, cameraFov, cameraZ }) {
    ...
    this.cameraFov = cameraFov; this.cameraZ = cameraZ; this.medias = []; this.isDown = false; this.loaded = 0
    AutoBind(this); this.createRenderer(); this.createCamera(); this.createScene()
    this.onResize(); this.createGeometry(); this.createMedias(); this.update(); this.addEventListeners()
  }
  ...
  createCamera() { this.camera = new Camera(this.gl); this.camera.fov = this.cameraFov; this.camera.position.z = this.cameraZ }
  createScene() { this.scene = new Transform() }
  createGeometry() { this.planeGeometry = new Plane(this.gl, { heightSegments: 1, widthSegments: 100 }) }
  createMedias() {
    this.medias = this.items.map((image, i) => new Media({
      gl: this.gl, geometry: this.planeGeometry, scene: this.scene, screen: this.screen,
      viewport: this.viewport, image, length: this.items.length, index: i,
  ...
  onTouchDown(e) { this.isDown = true; this.scroll.position = this.scroll.current; this.start = e instanceof TouchEvent ? e.touches[0].clientY : e.clientY }
  onTouchMove(e) { if (!this.isDown) return; const y = e instanceof TouchEvent ? e.touches[0].clientY : e.clientY; this.scroll.target = (this.scroll.position ?? 0) + (this.start - y) * 0.1 }
  ...
  update() {
    ...
    this.scroll.last = this.scroll.current; requestAnimationFrame(this.update)
  }

  addEventListeners() {
    window.addEventListener('resize', this.onResize); window.addEventListener('wheel', this.onWheel)
    window.addEventListener('mousedown', this.onTouchDown); window.addEventListener('mousemove', this.onTouchMove)
    window.addEventListener('mouseup', this.onTouchUp); window.addEventListener('touchstart', this.onTouchDown)
    window.addEventListener('touchmove', this.onTouchMove); window.addEventListener('touchend', this.onTouchUp)
  }

  destroy() {
    window.removeEventListener('resize', this.onResize); window.removeEventListener('wheel', this.onWheel)
    window.removeEventListener('mousedown', this.onTouchDown); window.removeEventListener('mousemove', this.onTouchMove)
    window.removeEventListener('mouseup', this.onTouchUp); window.removeEventListener('touchstart', this.onTouchDown)
    window.removeEventListener('touchmove', this.onTouchMove); window.removeEventListener('touchend', this.onTouchUp)
  }
}
...
export function FlyingPosters({
  ...
  const containerRef = useRef(null); const canvasRef = useRef(null); const instanceRef = useRef(null)
  ...
  return (
    <div ref={containerRef} className={`flying-posters${className ? ' ' + className : ''}`} {...props}>
      <canvas ref={canvasRef} className="flying-posters__canvas" />
    </div>
  )
}
